...
TEST(TransposeSinking, EdgeSplitting) {
  ...
  auto a = make_shared<opset::Parameter>(ng::element::i32, shape_nhwc);
  ...
  auto axes = make_shared<opset::Constant>(ng::element::i64, ng::Shape{4},
  ...
  auto new_transpose = ng::as_type_ptr<opset::Transpose>(
...
TEST(TransposeSinking, PoolAdd1) {
  ...
  auto const1 = opset::Constant::create(input_type, ng::Shape{1, 3, 3, 1},
                                        {3});  // NHWC (1,3,3,1)
...
TEST(TransposeSinking, PoolAdd3) {
  ...
  auto const1 =
      opset::Constant::create(input_type, ng::Shape{1}, {1});  // NHWC (1,3,3,1)
  auto add1 = make_shared<opset::Add>(transpose2, const1);
  auto func = make_shared<ng::Function>(add1, ng::ParameterVector{X});
...
TEST(TransposeSinking, Concat) {
  ...
  ng::Shape shape_nhwc{16, 28, 28, 1};
  auto a = make_shared<opset::Parameter>(ng::element::i32, shape_nhwc);
  auto b = make_shared<opset::Parameter>(ng::element::i32, shape_nhwc);
  ...
  auto concat =
      make_shared<opset::Concat>(ng::OutputVector{a_transpose, b_transpose}, 0);
...
TEST(TransposeSinking, Concat_DummyShape) {
  ...
  auto a1 = make_shared<opset::Parameter>(ng::element::i32, shape1);
  auto a2 = make_shared<opset::Parameter>(ng::element::i32, shape2);
  auto a3 = make_shared<opset::Parameter>(ng::element::i32, shape3);
  auto a4 = make_shared<opset::Parameter>(ng::element::i32, shape4);
...
TEST(TransposeSinking, Pad) {
  ...
  auto a = make_shared<opset::Parameter>(ng::element::f32, shape_nhwc);
  auto pad_value = opset::Constant::create<float>(ng::element::f32, ng::Shape{},
  ...
  ng::pass::Manager pass_manager;
  size_t before_count = count_ops_of_type<opset::Transpose>(f);
  pass_manager.register_pass<pass::TransposeSinking>();
  pass_manager.run_passes(f);
  size_t after_count = count_ops_of_type<opset::Transpose>(f);
  ASSERT_LE(after_count, before_count);
  auto result = f->get_results().at(0)->input_value(0).get_node_shared_ptr();
  ng::Shape expected_shape{1, 1, 1, 1};
  ASSERT_EQ(result->get_output_shape(0), expected_shape);
  auto out = ng::as_type_ptr<opset::ReduceSum>(
