...
if [ -z "${RUN_UID+x}" ] ; then
    ...
    #
    # Note 'dockuser' may be used in other scripts.  If you choose to change
    ...
    #
    # Note also that /home/dockuser may be used in other scripts.  If you
    # choose to change it here, then you need to change it in all other
    # scripts, or else the builds could break.
    #
    ...
    if [ -z "${RUN_CMD+x}" ] ; then  # Launch a shell as dockuser
      ...
      if [ ! -f "${RUN_CMD}" ] ; then
        ( >&2 echo "ERROR: Unable to find command ${RUN_CMD}" )
        exit 1
      fi
