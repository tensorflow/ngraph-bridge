...
class TestFloorOperations(NgraphTest):
    ...
    def test_cmns(self):
        ...
        with tf.Graph().as_default() as g:
            ...
            input_placeholders = [
                g.get_tensor_by_name("import/arg_" + str(i) + ':0')
                for i in range(6)
            ]

            outputs = [
                g.get_tensor_by_name("import/CombinedNonMaxSuppression:" +
                                     str(i)) for i in range(4)
            ]
            boxes = 19 * np.random.random([10, 2, 1, 4]) + 1
            scores = np.random.random([10, 2, 20])
            sess_fn = lambda sess: sess.run(
                outputs,
                feed_dict={
                    input_placeholders[0]: boxes,
                    input_placeholders[1]: scores,
