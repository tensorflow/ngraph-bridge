...
TEST_F(MarkForClusteringTest2, QueryBackendForSupportTest8) {
  ...
  ngraph::runtime::dummy::DummyBackend3 db;
  db.set_supported_behaviour({
      std::make_shared<ngraph::op::Abs>(), std::make_shared<ngraph::op::Exp>(),
...
// Y and M are not supported by backend, but after fusion YM is supported
// (X->YM->N).
// But N is not supported.
// So From X->Y->M->N, we get markings: X(T)->Y(T)->M(T)->N(F)  (T means true
// (supported) and viceversa)
// but because N is not supported, TF node B will not be marked.
// So we get a ng function X->Y
// But now we have to submit this again to the backend for inspection. Now the
// backend will say, oh wait, Y does not lead to M, hence I cannot support it.
// So now both A and B fall back to TF.

// To simulate the above scenario the is_supported of the test dummy backend
// will say it does not support "Multiply" the first time it is called
...
TEST_F(MarkForClusteringTest2, QueryBackendForSupportTest11) {
  ...
  for (auto node : g.nodes()) {
    ASSERT_EQ(
        (node->type_string() == "Abs" || node->type_string() == "Softplus"),
