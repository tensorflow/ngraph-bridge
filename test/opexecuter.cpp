...
void OpExecuter::ExecuteOnTF(vector<Tensor>& tf_outputs) {
  ...
  for (unsigned int i = 0; i < tf_outputs.size(); i++) {
...
void OpExecuter::ExecuteOnNGraph(vector<Tensor>& ngraph_outputs,
  ...
  for (int i = 0; i < number_of_inputs; i++) {
    ...
    for (unsigned int j = 0; j < src_nodes_metadata.size(); j++) {
  ...
  for (int i = 0; i < number_of_outputs; i++) {
    ...
    for (unsigned int j = 0; j < dest_nodes_metadata.size(); j++) {
  ...
  for (unsigned int i = 0; i < tf_inputs.size(); i++) {
    ng::Shape ng_shape;
    ASSERT_EQ(Status::OK(),
  ...
  for (auto i = 0; (unsigned)i < ng_function->get_output_size(); i++) {
