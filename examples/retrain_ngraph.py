...
def create_image_lists(image_dir, testing_percentage, validation_percentage):
  ...
  """
    ...
    for sub_dir in sub_dirs:
        ...
        extensions = sorted(
            set(
                os.path.normcase(ext)  # Smash case on Windows.
...
def get_bottleneck_path(image_lists, label_name, index, bottleneck_dir,
  ...
  """
    module_name = (
        module_name.replace('://', '~')  # URL scheme.
  ...
  """
    ...
    with tf.Graph().as_default() as graph:
        ...
        wants_quantization = any(
            node.op in FAKE_QUANT_OPS for node in graph.as_graph_def().node)
...
def add_input_distortions(flip_left_right, random_crop, random_scale,
  ...
  """
    ...
    resize_scale_value = tf.random_uniform(
        shape=[], minval=1.0, maxval=resize_scale)
    ...
    brightness_value = tf.random_uniform(
        shape=[], minval=brightness_min, maxval=brightness_max)
  ...
  """
    test_bottlenecks, test_ground_truth, test_filenames = (
        get_random_cached_bottlenecks(
            train_session, image_lists, FLAGS.test_batch_size, 'testing',
  ...
  """
    ...
    with eval_graph.as_default():
        ...
        (_, _, bottleneck_input, ground_truth_input,
         final_tensor) = add_final_retrain_ops(
             class_count,
    ...
    return (eval_sess, resized_input_tensor, bottleneck_input,
            ground_truth_input, evaluation_step, prediction)
...
def download_and_prepare():
    ...
    if not os.path.isdir('flower_photos'):
        cmd = 'wget http://download.tensorflow.org/example_images/flower_photos.tgz'
        if (call(shlex.split(cmd)) != 0):
            raise Exception("Error running command: " + cmd)

        cmd = 'tar xvf flower_photos.tgz'
        if (call(shlex.split(cmd)) != 0):
            raise Exception("Error running command: " + cmd)
...
def main(_):
    ...
    if not FLAGS.image_dir:
        ...
        # Now set the image_dir
        FLAGS.image_dir = './flower_photos'

        #tf.logging.error('Must set flag --image_dir.')
        tf.logging.info('Downloading the images and using the defauts')
    ...
    with tf.compat.v1.Session(
        ...
        elapsed = time.time()
        print("Time to train: %d seconds" % (elapsed - start))
