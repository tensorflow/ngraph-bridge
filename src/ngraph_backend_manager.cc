...
Status BackendManager::SetBackendName(const string& backend_name) {
  ...
  // TODO: Should NGRAPH_TF_BACKEND be read here as well and should it override
  // backend_name?
...
Status BackendManager::GetCurrentlySetBackendName(string* current_backend) {
  ...
  if (possible_backend.empty() ||
      !BackendManager::IsSupportedBackend(possible_backend)) {
    current_backend = nullptr;
    return errors::Internal("NGRAPH_TF_BACKEND: ", possible_backend,
                            " is not supported");
  } else {
