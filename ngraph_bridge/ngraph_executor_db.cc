...
NGraphExecutorDB::NGraphExecutorDB(int depth) : m_depth(depth) {}
...
bool NGraphExecutorDB::MaybeGetNgExecutable(
  ...
  ng_exec = it->second;
  // push recently used signature to the list
  UpdateLRU(signature);
  return true;
}
...
void NGraphExecutorDB::AddItem(
  ...
  if (!signature.empty() && ng_function && ng_exec) {
    ...
    // if maps are full, remove items from all three maps
    // and from the list
    ...
    // Insert into m_ng_exec_map and m_ng_function map
    // and into m_lru list
    ...
    if (ng_exec_map_output_pair.second) {
      ...
      auto it = m_executable_pipelined_tensors_map.find(ng_exec);
...
void NGraphExecutorDB::RemoveItem(
    const std::string signature,
    std::shared_ptr<ngraph::runtime::Executable>& evicted_ng_exec) {
  evicted_ng_exec = m_ng_exec_map.at(signature);
  m_ng_exec_map.erase(signature);
  m_ng_function_map.erase(evicted_ng_exec);
  m_lru.pop_back();
}

void NGraphExecutorDB::UpdateLRU(const std::string signature) {
