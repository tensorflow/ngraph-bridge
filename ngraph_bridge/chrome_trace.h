...
class trace::event::Duration {
 public:
  explicit Duration(const std::string& name, const std::string& category,
                    const std::string& args = "");
  ~Duration() { write(); }
  /// \brief stop the timer without writing the data to the log file. To write
  /// the data
  ...
  /// This funtion has an implicit stop() if stop() has not been previously
  /// called
  void write();
