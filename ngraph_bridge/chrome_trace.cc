...
static bool read_tracing_env_var() {
  static const bool is_enabled = ngraph::getenv_bool("NGRAPH_ENABLE_TRACING");
...
event::Duration::Duration(const string& name, const string& category,
                          const string& args) {
...
void event::Duration::write() {
  if (Manager::is_tracing_enabled()) {
    size_t stop_time =
        (m_stop != 0 ? m_stop : Manager::get_current_microseconds());
    ...
    str +=
        R"({"name":")" + m_name + R"(","cat":")" + m_category +
        R"(","ph":"X","pid":)" + Manager::get_process_id() + R"(,"tid":)" +
        Manager::get_thread_id() +
        R"(,"ts":)" + to_string(m_start) + R"(,"dur":)" +
        to_string(stop_time - m_start);
    if (!m_args.empty()) {
      str += R"(,"args":)" + m_args;
    }
    str += "}";
...
void event::Object::write_snapshot(ostream& out, const string& args) {
  string str = R"({"name":")" + m_name + R"(","ph":"O","id":")" +
               to_string(m_id) +
               R"(","ts":)" + to_string(Manager::get_current_microseconds()) +
               R"(,"pid":)" + Manager::get_process_id() + R"(,"tid":)" +
               Manager::get_thread_id();
...
void event::Object::destroy() {
  if (Manager::is_tracing_enabled()) {
    ...
    string str = R"({"name":")" + m_name + R"(","ph":"D","id":")" +
                 to_string(m_id) +
                 R"(","ts":)" + to_string(Manager::get_current_microseconds()) +
                 R"(,"pid":)" + Manager::get_process_id() + R"(,"tid":)" +
                 Manager::get_thread_id() + "}";
...
void event::Manager::enable_event_tracing() { s_tracing_enabled = true; }

void event::Manager::disable_event_tracing() { s_tracing_enabled = false; }

bool event::Manager::is_event_tracing_enabled() { return s_tracing_enabled; }
