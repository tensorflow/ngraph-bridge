...
std::pair<Status, T> LookUpOrCreate(std::string key, 
                                    std::function<std::pair<Status, T>()> create_item,
...
std::pair<Status, T> NgraphDataCache<T>::LookUpOrCreate(
  ...
  std::function <void(T)> callback_destroy_item)
 {
  bool found_in_cache;
   ...
   return std::make_pair(errors::Internal("Failed to create value for requested key"),items);
}
}
