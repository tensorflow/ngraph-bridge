...
const std::map<std::string, std::set<shared_ptr<ng::Node>>>& GetTFToNgOpMap() {
  static bool initialized = false;
  ...
  if (!initialized) {
    ...
    static std::map<std::string, std::set<shared_ptr<ng::Node>>>
        TFtoNgraphOpMap{
...
Status MarkForClustering(Graph* graph, const std::set<string> skip_these_nodes,
  ...
  // This map is not const, since it can change based on some backend specific
  // op support.
  std::map<std::string, ConfirmationFunction> confirmation_function_map =
      GetConfirmationMap();

  const std::map<std::string, std::set<shared_ptr<ng::Node>>>& TFtoNgraphOpMap =
      GetTFToNgOpMap();
