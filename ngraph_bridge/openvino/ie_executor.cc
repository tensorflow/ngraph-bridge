...
IE_Executor::IE_Executor(InferenceEngine::CNNNetwork ie_network, std::string device)
...
void IE_Executor::infer(std::vector<std::shared_ptr<IE_Data>> inputs,
    ...
    //int batch_size = IEManager::GetInputBatchSize(input_shapes[0][0], m_device);
    int batch_size = IEManager::GetInputBatchSize(inputs[0]->get_shape()[0], m_device);
    int num_req = inputs[0]->get_shape()[0]/batch_size;
    std::cout << "Running Inference - Device: " << m_device << ", Num Req: " << num_req << ", Batch Size: " << inputs[0]->get_shape()[0] << ", Req Batch Size: " << batch_size << std::endl;
    ...
    std::vector<InferenceEngine::MemoryBlob::Ptr> in_blobs(num_req*inputs.size());
    std::vector<InferenceEngine::MemoryBlob::Ptr> out_blobs(num_req*outputs.size());
    std::vector<InferenceEngine::MemoryBlob::Ptr> param_blobs(hoisted_params.size());
	...
	std::string input_name = inputs[i]->has_name() ? inputs[i]->get_name() : it.first;
        ...
        for (int j=0; j<num_req; j++) {
            ...
            const void *data_ptr = (void*)((uint64_t)(input_data_pointer)+req_size*j);
	    int in_idx = i*num_req+j;
            IEManager::CreateBlob(desc, input_precision, data_ptr, req_size, in_blobs[in_idx]);
	...
	std::string param_name = hoisted_params[i]->get_name();
        ...
        IEManager::CreateBlob(desc, param_precision, param_data_pointer, size, param_blobs[i]);
        for (int j=0; j<num_req; j++) {
	...
	std::string output_name = outputs[i]->has_name() ? outputs[i]->get_name() : it.first;

        InferenceEngine::SizeVector req_shape(output_shape);
        req_shape[0] = batch_size;
        InferenceEngine::TensorDesc desc(output_precision, req_shape, output_layout);

        for (int j = 0; j<num_req; j++) {
            size_t req_size = size/num_req;
            const void *data_ptr = (void*)((uint64_t)(output_data_pointer)+req_size*j);
	    int out_idx = i*num_req+j;
            IEManager::CreateBlob(desc, output_precision, data_ptr, req_size, out_blobs[out_idx]);
...
void IE_Executor::complete_async_inference(const int req_id) {
  ...
  try {
    ...
    m_infer_reqs[req_id].Wait(InferenceEngine::IInferRequest::WaitMode::RESULT_READY);
...
size_t IE_Executor::getOutputBatchSize(size_t inputBatchSize, std::string device) const {
    return m_network.getBatchSize()*IEManager::GetNumRequests(inputBatchSize, device);
}
