...
using ReshapeMap = unordered_map<shared_ptr<ng::Node>, shared_ptr<ng::op::Reshape>>;

static string describe_reshape(shared_ptr<ng::Node> node)
{
    stringstream ss;
    auto reshape = ng::as_type_ptr<ng::op::Reshape>(node);
    ss << reshape->get_name()
       << " ( axis order = " << ng::vector_to_string(reshape->get_input_order())
...
static shared_ptr<ng::op::Reshape>
    make_reshape(shared_ptr<ng::Node> arg, const ng::AxisVector& input_order, const ng::Shape& output_shape)
{
    auto reshape = make_shared<ng::op::Reshape>(arg, input_order, output_shape);
    NGRAPH_DEBUG << "Make Reshape " << describe_reshape(reshape);
    return reshape;
}

static void
    write_reshapemap(ReshapeMap& reorders, shared_ptr<ng::Node> target, shared_ptr<ng::op::Reshape> reshape)
{
...
static shared_ptr<ng::op::Reshape> read_reshapemap(ReshapeMap& reorders, shared_ptr<ng::Node> target)
{
...
static shared_ptr<ng::op::Reshape> combine_reshapes(shared_ptr<ng::op::Reshape> r1,
                                                shared_ptr<ng::op::Reshape> r2)
{
    auto default_order = ng::get_default_order(r1->get_shape());
    auto perm_r1 = ng::apply_permutation(default_order, r1->get_input_order());
    auto perm_r2 = ng::apply_permutation(perm_r1, r2->get_input_order());
...
static void insert_reshape(shared_ptr<ng::Node> target, shared_ptr<ng::Node> reshape, size_t input_index)
{
...
static void delete_reshape(shared_ptr<ng::Node> reshape)
{
    NGRAPH_DEBUG << "Removing reshape " << reshape->get_name();
    if (!reshape->get_users().empty())
    {
        ng::replace_node(reshape, reshape->get_argument(0));
    }
}

static void mark_reshape_for_deletion(shared_ptr<ng::Node> reshape,
...
static shared_ptr<ng::op::Reshape> create_default_reshape(shared_ptr<ng::Node> n)
{
...
static ng::AxisSet get_quantization_axes_in_default_order(shared_ptr<ng::op::Reshape> arg_reshape,
                                                      const ng::AxisSet& old_axis_set)
{
    auto perm_to_def = ng::get_permutation_to_default_order(arg_reshape->get_input_order());
    ng::AxisSet axis_set;
...
struct Swimmer
{
    ...
    shared_ptr<ng::op::Reshape> reshape;
};
...
// as possible
void swim(ng::Input<ng::Node> input, shared_ptr<ng::op::Reshape> reshape)
{
    ...
    while (work_queue.size() > 0)
    {
        ...
        else if (ng::is_type<ng::op::Broadcast>(n))
        {
            auto old_broadcast = static_pointer_cast<ng::op::Broadcast>(n);
            ...
            if (!in_order)
            {
                ng::AxisVector new_source_axes_sorted{new_source_axes};
                ...
                ng::AxisVector new_source_axis_order;
                ...
                auto new_arg_shape =
                    ng::apply_permutation(broadcast_input->get_shape(), new_source_axis_order);
                broadcast_input =
                    make_reshape(broadcast_input, new_source_axis_order, new_arg_shape);
            }

            auto new_broadcast = make_shared<ng::op::Broadcast>(
...
// as far as we can
static void convert_binary_to_default_order(shared_ptr<ng::Node> binary,
                                            const ng::Input<ng::Node>& input,
                                            shared_ptr<ng::Node> right,
                                            ReshapeMap& reorders,
                                            set<shared_ptr<ng::Node>>& reshapes_to_delete)
{
    auto left = input.get_source_output().get_node_shared_ptr();
    auto perm_to_def =
        ng::get_permutation_to_default_order(reorders.at(right)->get_input_order());
    auto new_shape = ng::apply_permutation(left->get_shape(), perm_to_def);
    NGRAPH_DEBUG << "right = " << ng::vector_to_string(right->get_shape()) << ", "
                 << right->get_name();
...
static void materialize_shapes(shared_ptr<ng::Node> n,
                               ReshapeMap& reorders,
                               set<shared_ptr<ng::Node>>& reshapes_to_delete)
{
...
static void sink_reshape(shared_ptr<ng::op::Reshape> reshape,
                         ReshapeMap& reorders,
                         set<shared_ptr<ng::Node>>& reshapes_to_delete)
{
...
static void sink_binary(shared_ptr<ng::Node> binary,
                        ReshapeMap& reorders,
                        set<shared_ptr<ng::Node>>& reshapes_to_delete)
{
    ...
    else if (reorders.at(left)->get_input_order() == ng::get_default_order(left->get_shape()))
    {
        convert_binary_to_default_order(
            binary, binary->input(0), right, reorders, reshapes_to_delete);
    }
    else if (reorders.at(right)->get_input_order() == ng::get_default_order(right->get_shape()))
    {
        convert_binary_to_default_order(
...
static void sink_slice(shared_ptr<ng::op::Slice> n,
                       ReshapeMap& reorders,
                       set<shared_ptr<ng::Node>>& /* reshapes_to_delete */)
{
    ...
    auto input_shape = ng::apply_permutation(arg_reshape->get_shape(), def_order);
    auto dummy_correct_shape =
        make_shared<ng::pattern::op::Label>(arg_reshape->get_element_type(), input_shape);

    auto new_lower = ng::apply_permutation(n->get_lower_bounds(), def_order);
    auto new_upper = ng::apply_permutation(n->get_upper_bounds(), def_order);
    auto new_strides = ng::apply_permutation(n->get_strides(), def_order);
    auto new_slice = make_shared<ng::op::Slice>(dummy_correct_shape, new_lower, new_upper, new_strides);
    ng::replace_node(dummy_correct_shape, n->get_argument(0));
    NGRAPH_DEBUG << "Replacing " << n->get_name() << " with " << new_slice->get_name();
    ng::replace_node(n, new_slice);
...
static void sink_pad(shared_ptr<ng::op::Pad> n,
                     ReshapeMap& reorders,
                     set<shared_ptr<ng::Node>>& /* reshapes_to_delete */)
{
    auto arg_reshape = reorders.at(n->get_argument(0));
...
static void sink_quantize(shared_ptr<ng::op::Quantize> quantize,
                          ReshapeMap& reorders,
                          set<shared_ptr<ng::Node>>& /* reshapes_to_delete */)
{
    auto arg_reshape = reorders.at(quantize->get_argument(0));
    ng::AxisSet axes_in_def_order =
        get_quantization_axes_in_default_order(arg_reshape, quantize->get_axes());
    auto new_quantize = make_shared<ng::op::Quantize>(quantize->get_argument(0),
    ...
    ng::replace_node(quantize, new_quantize);
    write_reshapemap(reorders, new_quantize, arg_reshape);
}

static void sink_concat(shared_ptr<ng::op::Concat> n,
                        ReshapeMap& reorders,
                        set<shared_ptr<ng::Node>>& reshapes_to_delete)
{
    auto arg_reshape = reorders.at(n->get_argument(0));
    ...
    for (size_t i = 1; i < n->get_input_size(); i++)
    {
        ...
        auto iinput_shape = ng::apply_permutation(iarg_reshape->get_shape(), def_order);
        auto idummy_correct_shape =
            make_shared<ng::pattern::op::Label>(iarg_reshape->get_element_type(), iinput_shape);
        new_args.push_back(idummy_correct_shape);
    }

    auto new_axis = order.at(n->get_concatenation_axis());
    auto new_concat = make_shared<ng::op::Concat>(new_args, new_axis);
    // put back the original arguments
    for (size_t i = 0; i < new_concat->get_input_size(); i++)
    {
        ng::replace_node(new_args.at(i), n->get_argument(i));
    }
    NGRAPH_DEBUG << "Replacing " << n->get_name() << " with " << new_concat->get_name();
    ng::replace_node(n, new_concat);
...
static void sink_dequantize(shared_ptr<ng::op::Dequantize> dequantize,
                            ReshapeMap& reorders,
                            set<shared_ptr<ng::Node>>& /* reshapes_to_delete */)
{
    ...
    auto new_dequantize = make_shared<ng::op::Dequantize>(dequantize->get_argument(0),
                                                      dequantize->get_argument(1),
                                                      dequantize->get_argument(2),
                                                      dequantize->get_element_type(),
                                                      axes_in_def_order);

    ng::replace_node(dequantize, new_dequantize);
