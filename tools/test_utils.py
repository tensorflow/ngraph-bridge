...
class PyTestManifestParser:

    def get_test_list(tf_path, test_regex):
        accepted_formats = [
            "*test*", "math_ops_test.DivNoNanTest.testBasic",
            "math_ops_test.DivNoNanTest.*", "math_ops_test.D*",
            "math_ops_test.*", "math_*_test", "math_*_*_test", "math*_test"
        ]
        try:
    ...
    def list_tests(module_list, regex_input):
        ...
        for test_module in module_list:
            try:
                ...
                print("Exception in __import__({})".format(test_module),
                      'ERROR:', str(e))
            ...
            try:
                ...
                print(
                    "Exception in loader.loadTestsFromModule({})".format(
                        moduleobj), 'ERROR:', str(e))
                module_list.remove(test_module)
                continue
            ...
            for line in fh.readlines():
                ...
                if curr_section == 'import_section':
                    ...
                    if line in g_imported_files:
                        sys.exit("ERROR: re-import of manifest " + line +
                                 " in " + manifestfile)
                ...
                if curr_section == 'run_section':
                    list_of_tests.extend(
                        get_test_list(tensorflow_path, line)[0])
...
def ReturnGTestListFromManifest(manifestfile, path):
    # TODO
    parser = PyTestManifestParser()
