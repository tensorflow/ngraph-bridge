...
with tf.Session() as sess:
    res = sess.run(graph_in_out_nodes['outputs'], feed_dict = {k:np.zeros((10,)) for k in graph_in_out_nodes['inputs']})
    print(res)
    builder.add_meta_graph_and_variables(sess, [tf.saved_model.tag_constants.TRAINING])
    builder.add_meta_graph([tf.saved_model.tag_constants.SERVING], strip_default_attrs=True)
builder.save()

tf.io.write_graph(sess.graph.as_graph_def(), '.', 'test_graph_SM.pbtxt', as_text=True)