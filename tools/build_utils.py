...
def download_repo(target_name, repo, version, submodule_update=False):
    ...
    curr_remote = subprocess.check_output("git remote get-url origin", shell=True).rstrip()
    if curr_remote != repo:
    ...
    is_branch = ""
    try:
        is_branch = subprocess.check_output("git show-ref --verify refs/heads/" + version, shell=True).rstrip()
    except:
        pass
    if is_branch == "":
        # we were given a tag/commit ref
        curr_git_branch = subprocess.check_output("git branch | grep '*' | sed -e 's#.* ##'", shell=True).rstrip()
